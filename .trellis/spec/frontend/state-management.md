...
- Node-only migration API: `POST /api/storage/team/migrations`, `runtime = "nodejs"`, requires a trusted origin and `x-imagine-setup-token` matching `IMAGINE_TEAM_SETUP_TOKEN`; invalid setup-token attempts use the shared setup-token rate-limit policy and return generic migration failures before database access.
