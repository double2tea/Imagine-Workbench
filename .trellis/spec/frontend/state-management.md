...
- Node-only migration API: `POST /api/storage/team/migrations`, `runtime = "nodejs"`, requires a trusted origin and `x-imagine-setup-token` matching `IMAGINE_TEAM_SETUP_TOKEN`; invalid setup-token attempts use the shared setup-token rate-limit policy and return generic migration failures before database access. Scripted deployments may call the same API through `pnpm db:migrate`, which requires `APP_URL` and `IMAGINE_TEAM_SETUP_TOKEN` and must print only non-secret schema/migration counts.
