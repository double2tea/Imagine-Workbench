...
DaVinci Resolve integration starts with [DaVinci Resolve Bridge](docs/resolve-bridge.md). The integration runs inside Resolve through the Workflow Integration panel (`Workspace → Workflow Integrations → Imagine Workbench`), using a local, LAN, or deployed Imagine Workbench endpoint.
...
- `GET /v1/models?provider=all|<key>&kind=chat|image|video|audio|all`: OpenAI-compatible model list response with provider-prefixed model IDs. `provider=all` defaults to the v1-callable Workbench-known catalog and excludes async/workflow/video-only media targets; use `/api/models` for the richer app UI catalog.
...
- Built-in model capabilities come from `lib/providers/catalog/data/model-capabilities.json`, with public/selectable entries derived through `getListedModelCapabilities()`. `GET /api/model-capabilities` exposes the complete capability catalog for Workbench clients and integrations. The "获取模型" button fetches the live model list from each provider's `/v1/models` endpoint and merges eligible entries into the dropdowns: live/manual chat models may be added dynamically, while live/manual image, video, and audio models must be catalog-known/listed or explicitly mapped to a known capability. Media generation capability is not guessed from model names.
