...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemInstruction =
      ...
      "## Audio Planning\n" +
      "For script or video-production requests, plan audio as first-class media: narration/dialogue uses audioMode tts, described custom voices use voice_design with audioStylePrompt, authorized reference-voice work uses voice_clone with an audio reference, background score uses music, and impact/ambient cues use sfx. Prefer create_board_audio_flow on board and generate_audio on the main workbench. Do not invent unsupported ASR/music/clone capabilities without first calling query_models({kind:\"audio\"}).\n\n" +
