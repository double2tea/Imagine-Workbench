...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const options = parseTeamSettingListOptions(new URL(request.url).searchParams);
    const result = await withPostgresClient(config, client => listTeamSettings(client, config, request, options));
    return Response.json(result);
  } catch (error) {
    const response = apiErrorResponse(error, "Team setting list failed");
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const input = await readTeamSettingRequestJson(request);
    const config = resolvePostgresStorageConfig(process.env);
    const result = await withPostgresClient(config, client => saveTeamSetting(client, config, request, input));
    return Response.json(result);
  } catch (error) {
    const response = apiErrorResponse(error, "Team setting save failed");
    return Response.json(response.body, {
      status: error instanceof PostgresStorageConfigError ? 400 : response.status,
    });
  }
...
function parseTeamSettingListOptions(searchParams: URLSearchParams): TeamSettingListOptions {
  return {
...
function groupParams(searchParams: URLSearchParams): WorkspaceSettingGroup[] | undefined {
  ...
  return groups.map(group => {
    ...
    throw badRequest("Invalid team setting group", "invalid_team_setting_query");
  });
}
